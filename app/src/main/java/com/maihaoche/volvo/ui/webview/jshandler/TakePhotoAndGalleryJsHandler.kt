 ...
 * Desc：
 */
class TakePhotoAndGalleryJsHandler : JsHandler {
    ...
    override fun OnHandler(handlerName: String?, responseData: String?, function: CallBackFunction?) {
        responseData?.let {
            try {
                ...
                mCallBack.takePhotoAndGallery(json.getString("type").toInt()
                        , json.getString("max").toInt()
                        , json.getString("isCompress") == "1",function)
            } catch (e: Exception) {
