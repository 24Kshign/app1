...
public class QiNiuUtil1 {
    ...
    public static CancleUpload getInstence(){
        if(cancel == null){
            cancel = new QiNiuUtil1().new CancleUpload();
        }
        return cancel;
    }
    ...
    public static void getToken(BaseFragmentActivity activity, TokenSucListener listener, TokenFailListener tokenFailListener){
        ...
        public static void uploadImg(BaseFragmentActivity activity,String url ,Listener listener,CancleUpload cancel){
        if( token== null){
        AppApplication.getServerAPI().getToken()
