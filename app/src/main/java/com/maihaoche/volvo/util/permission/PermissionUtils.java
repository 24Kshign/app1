...
class PermissionUtils {
    ...
    static void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        PermissionHandler.PermissionRequestInterface requestInterface = removePermissionRequest(requestCode);
        if (requestInterface != null) {
            requestInterface.onPermissionGranted(isAllPermissionGranted);
        }
    }
            ...
            //当前没有权限正在被申请
                ...
                if (newRequest) {
                    ...
                    for (String aPermission : permission) {
                        //有某个权限未被授权
                        int permissionGrantValue = ActivityCompat.checkSelfPermission(activity, aPermission);
                        if (permissionGrantValue != PackageManager.PERMISSION_GRANTED) {
                            isAllPermissionGranted = false;
                        }
                        //有某个权限未被授权,并且勾选了"不再提示".则直接返回结果
                        boolean shouldShow = ActivityCompat.shouldShowRequestPermissionRationale(activity, aPermission);
                        if (shouldShow) {
                            removePermissionRequest(requestCode);
                            requestInterface.onPermissionGranted(false);
                            return false;
                        }
                    }
    ...
    private static void requstPermissionWithHint(final Activity context
        ...
        for (String aPermission : permission) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(context, aPermission)) {
                shouldShowHint = true;
            }
        }
        //权限之前被禁用过。需要弹窗提示。
     ...
     * @param requestCode 请求编码
     */
    private static PermissionHandler.PermissionRequestInterface removePermissionRequest(int requestCode) {
        if (mRquestCodeInProcess == requestCode) {
            mRquestCodeInProcess = -1;
        }
        permissionObservable.notifyObservers();
        return mPermissionRequests.remove(requestCode);
    }
}
